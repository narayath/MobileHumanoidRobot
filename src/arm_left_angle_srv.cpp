...
{
    ros::init(argc, argv, "arm_left_angle_srv");   
    ros::NodeHandle n;    
    ...
    ifstream pointsFile;
    pointsFile.open("/home/tuttu/ros_ws/src/pluto/src/goodLeftPointsList.txt");
    ...
    while(ros::ok())
    {
	   while(getline(pointsFile, line)) 
	   {
		   ...
		   srv.request.arm = 1; //0 is right arm, 1 is left arm
