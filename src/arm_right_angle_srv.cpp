...
pluto::hand_motion_msg hand_srv;
...
{
    ros::init(argc, argv, "arm_angle_srv");   
    ros::NodeHandle n;    
    client = n.serviceClient<pluto::arm_angles_msg>("robot_controller");
    hand_client = n.serviceClient<pluto::hand_motion_msg>("hand_controller");
    ...
    while(ros::ok())
    {
	   while(getline(pointsFile, line)) 
	   {
		   ...
		   hand_srv.request.arm = 0; //0 is right arm, 1 is left arm
	    	   hand_srv.request.handMotion = 1; //0 is close, 1 is open
	    	   
		   if(hand_client.call(hand_srv))
		   {
			if(hand_srv.response.pos_ok)
			{
			}
			else
			{
		   ...
		   {
			ROS_ERROR("Failed to call service hand_controller");
		   }
		   sleep(1);
		   ...
		   if(client.call(srv))
		   {
                  ...
                  hand_srv.request.arm = 0; //0 is right arm, 1 is left arm
	    	   hand_srv.request.handMotion = 0; //0 is close, 1 is open
